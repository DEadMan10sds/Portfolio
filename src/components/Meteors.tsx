...
export function StarfieldBackground({
  ...
  return (
    ...
    >
      ...
      {/* BACKGROUND grid */}
      <svg className="absolute inset-x-0 -top-[1rem] -z-10 h-[80%] w-full skew-y-12 fill-black/[.06] stroke-black/[.06] dark:fill-white/[.06] dark:stroke-white/[.06] [mask-image:linear-gradient(to_bottom_right,white,transparent,transparent)]">
        <defs>
          <pattern
            id={patternId}
            width="80"
      ...
      <div ref={starsRef} className="absolute inset-0">
        {Array.from({ length: stars }).map((_, i) => (
          <div key={i} data-star className="absolute rounded-full bg-white" />
        ))}
      </div>
